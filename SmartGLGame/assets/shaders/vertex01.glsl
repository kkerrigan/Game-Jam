...
in mat4 instanceMatrix; //this is 4 x vec4
...
uniform sampler2D texture00;	//used for height maps
uniform sampler2D texture01;	//used for height maps
...
void main()
{
	...
	if ( bUseHeightMap )
	{
		...
		// Returns 0.0 to 1.0
		float height = texture( texture00, vUV_x2.st ).r;
		
		height = height;
		
		//posTemp.y = 0.0f;		// "Flatten" the mesh
		posTemp.y = height * 60.0;		// Set the heigth
		// You could also "adjust" an existing mesh
	
	}//if ( bUseHeightMap )
	
	if(bUseWaterHeightMaps){
	
	
		float height = texture( texture00, vUV_x2.st + waterOffsets.xy ).r;
		
		height = height * 0.025 - 0.20;
	...
	if(is_instanced){
		...
		gl_Position = matProj * matView * instanceMatrix * vec4(posTemp, 1.0);
		
		
		vertPosWorld = instanceMatrix * vec4(posTemp, 1.0f);
		vertNormal = inverse(transpose(instanceMatrix)) * vec4(normalize(vNormal.xyz),1.0f);
	...
	vec3 FragPos = vec3(matModel * vec4(posTemp, 1.0));
