...
uniform bool bSecondPass;
uniform sampler2D texPass1OutputTexture;

// Cube map texture (NOT a sampler3D)
uniform samplerCube night_skybox_texture;
uniform samplerCube dawn_skybox_texture;
uniform samplerCube morning_skybox_texture;
uniform samplerCube day_skybox_texture;
uniform samplerCube dusk_skybox_texture;
uniform vec4 skybox_weights[2];
...
uniform bool render_target_overlay;

uniform bool is_h_bloom;
...
uniform bool use_discard_texture;
uniform bool bUseSplatMap;
uniform bool bUseWaterHeightMaps;
uniform vec4 waterOffsets;
...
void main()
{
	...
	if ( int(renderPassNumber) == 2 || render_target_overlay)
	{ 
		if(render_target_overlay){
			finalOutputColour.rgb = texture( texture00, vertUV_x2.st ).rgb;
	...
	{ 
		...
		vec3 vertexMaterialColour = texture( texPass1OutputTexture, textCoords.st ).rgb;
		...
		for ( float x = -size; x <= size; x += step )
		{
			...
			{
				count++;
				vecSample += texture( texPass1OutputTexture, textCoords.st + vec2(x, y)).rgb;
			}
		}
	...
	if(use_discard_texture){
		vec4 tex_sample = texture( texture00, vertUV_x2.st ).rgba;
	...
	if (useSkyBoxTexture == true)
	{
		...
		vec3 skyPixelColour = 
			texture( night_skybox_texture, vertNormal.xyz ).rgb * skybox_weights[0].x + 
			texture( dawn_skybox_texture, vertNormal.xyz ).rgb * skybox_weights[0].y + 
			texture( morning_skybox_texture, vertNormal.xyz ).rgb * skybox_weights[0].z + 
			texture( day_skybox_texture, vertNormal.xyz ).rgb * skybox_weights[0].a + 
			texture( dusk_skybox_texture, vertNormal.xyz ).rgb * skybox_weights[1].x;
	...
	if ( bAddReflect )
	{
		...
		vec3 vReflect = reflect(vertNormal.xyz, viewVector);
		
		rgbReflect = texture( night_skybox_texture, vReflect ).rgb;
	}//if ( bAddReflect )
	...
	if (bAddRefract)
	{
		...
		// Calcualte the reflection vector
		vec3 vRefract = refract(viewVector, normalize(vertNormal.xyz), 1.5);
		
		rgbRefract = texture( night_skybox_texture, vRefract ).rgb;
	}//if (bAddRefract)	
	...
	if ( bAddReflect || bAddRefract )
	{
		...
		finalOutputColour.a = objectDiffuse.a;	
		return;	
	}
...
//		materialDiffuse = objectDiffuse;
		...
		if ( !bUseSplatMap && !bUseWaterHeightMaps) //not the terrain
		{
		materialDiffuse =  objectDiffuse
		...
		else if(bUseWaterHeightMaps){
		
			materialDiffuse =  objectDiffuse + (texture( texture00, vertUV_x2.st + waterOffsets.xy ).rgba * 0.25);
		}
		
		else
		{
			vec4 weights = texture( texture00, vertUV_x2.st ).rgba;
			
			
		
			materialDiffuse = texture(texture01, vertUV_x2.st).rgba * weights.r +
							  texture(texture02, vertUV_x2.st).rgba * weights.g + 
							  texture(texture03, vertUV_x2.st).rgba * weights.b;
		}
	}//if ( useVertexColour )
...
//theLights[0].atten;
	...
	{	
		...
		if ( intLightType == DIRECTIONAL_LIGHT_TYPE )		// = 2
		{
			...
			finalObjectColour.rgb += (materialDiffuse.rgb * theLights[index].diffuse.rgb * lightContrib * 0.75); 
