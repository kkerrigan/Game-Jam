...
cSimpleAssimpSkinnedMesh::cSimpleAssimpSkinnedMesh(void) {
	...
	InitializeCriticalSection(&animation_map_lock);
...
cSimpleAssimpSkinnedMesh::~cSimpleAssimpSkinnedMesh(void) {
	...
	DeleteCriticalSection(&animation_map_lock);
	this->shutErDown();
...
void cSimpleAssimpSkinnedMesh::GetListOfBoneIDandNames(std::vector<std::string>& vecBoneNames, const std::string animationName)
{

	std::map<std::basic_string<char>, sAnimationInfo>::iterator itAnimation = this
		->mapAnimationFriendlyNameTo_pScene.find(
			animationName);
	...
	for (unsigned int chanID = 0;
		chanID != itAnimation->second.pAIScene->mAnimations[0]->mNumChannels;
		chanID++)
	{
		std::string boneName;
		boneName.append(itAnimation->second.pAIScene->mAnimations[0]->mChannels[chanID]->mNodeName.data);
...
void cSimpleAssimpSkinnedMesh::add_new_animation_to_map(sAnimationInfo info)
{
	EnterCriticalSection(&animation_map_lock);
	this->mapAnimationFriendlyNameTo_pScene[info.friendlyName] = info;
	LeaveCriticalSection(&animation_map_lock);
}
...
float cSimpleAssimpSkinnedMesh::findAnimationTotalTime(std::string animationName) {
	...
	double duration = (float)itAnimation->second.pAIScene->mAnimations[0]->mDuration;
	LeaveCriticalSection(&animation_map_lock);
	return duration;
}
...
float cSimpleAssimpSkinnedMesh::findAnimationTicks(std::string animationName)
{
	...
	double ticks = itAnimation->second.pAIScene->mAnimations[0]->mTicksPerSecond;
	LeaveCriticalSection(&animation_map_lock);
...
struct sAniThreadInfo
{
	std::string filename;
	std::string friendly_name;
	cSimpleAssimpSkinnedMesh* skinned_mesh;
};

DWORD WINAPI loadMeshAnimationThreaded_work(void* data)
{
	
	sAniThreadInfo* animation_info = (sAniThreadInfo*)(data);

	unsigned int Flags = aiProcess_Triangulate | aiProcess_OptimizeMeshes | aiProcess_OptimizeGraph | aiProcess_JoinIdenticalVertices;

	Assimp::Importer* pImporter = new Assimp::Importer();

	cSimpleAssimpSkinnedMesh::sAnimationInfo animInfo;
	animInfo.friendlyName = animation_info->friendly_name;
	animInfo.fileName = animation_info->filename;
	animInfo.pAIScene = pImporter->ReadFile(animInfo.fileName.c_str(), Flags);
	...
	animation_info->skinned_mesh->add_new_animation_to_map(animInfo);

}



bool cSimpleAssimpSkinnedMesh::loadMeshAnimation_Threaded(const std::string& friendlyName, const std::string& filename)
{
	// Already loaded this ?
	std::map< std::string /*animation FRIENDLY name*/,
		sAnimationInfo >::iterator itAnimation = this->mapAnimationFriendlyNameTo_pScene.find(friendlyName);
	...
	sAniThreadInfo* animation_info = new sAniThreadInfo();
	animation_info->filename = filename;
	animation_info->friendly_name = friendlyName;
	animation_info->skinned_mesh = this;
	...
	hThreadHandle = CreateThread(NULL,	// Default security
		...
		&loadMeshAnimationThreaded_work, // Pointer to the thread's function
		(void*)animation_info,		// The value (parameter) we pass to the thread
...
void cSimpleAssimpSkinnedMesh::boneTransform(float TimeInSeconds,
	...
	float TicksPerSecond = findAnimationTicks(animationName);
	...
	this->GetListOfBoneIDandNames(names, animationName);
...
cMesh* cSimpleAssimpSkinnedMesh::createMeshObjectFromCurrentModel(unsigned int meshIndex /*=0*/) {
	...
	if (this->pScene && this->pScene->mNumMeshes < meshIndex) {	// Doesn't have this mesh
		return NULL;
	}
